...
extension MapVC: MKMapViewDelegate {
    ...
    @objc func dropPin(_ sender: UITapGestureRecognizer) {
        ...
        photos = []  //clear them out
        PhotoService.instance.photos = []
        ...
        PhotoService.instance.retrieveUrls(forAnnotation: annotation) { (finished) in
            if finished {
                self.photos = PhotoService.instance.photos
    ...
    func retrieveImages(completion: @escaping (_ status: Bool) -> ()) {
        ...
        for var photo in photos {
            Alamofire.request(photo.imageUrl).responseImage(completionHandler: { (response) in
                ...
                self.progressLbl?.text = "\(newPhotos.count)/30 images downloaded"
...
extension MapVC: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "photoCell", for: indexPath) as? PhotoCell {
            ...
            let imageView = UIImageView(image: selectedPhoto.photoImage)
...
extension MapVC: UIViewControllerPreviewingDelegate {
    ...
    func previewingContext(_ previewingContext: UIViewControllerPreviewing, viewControllerForLocation location: CGPoint) -> UIViewController? {
        //when you push all the way through to present the VC
        ...
        //gets sourceRect from sourceView
        //called this in viewDidLoad: registerForPreviewing(with: self, sourceView: collectionView!)
        previewingContext.sourceRect = cell.contentView.frame
        return popVC
    }
    
    func previewingContext(_ previewingContext: UIViewControllerPreviewing, commit viewControllerToCommit: UIViewController) {
        //this is the peak part with a slight press
